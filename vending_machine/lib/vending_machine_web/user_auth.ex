defmodule VendingMachineWeb.UserAuth do
  ...
  def log_out_user(conn) do
    ...
    if live_socket_id = get_session(conn, :live_socket_id) do
      VendingMachineWeb.Endpoint.broadcast(live_socket_id, "disconnect", %{})
    end
  ...
  the current_user:
      ...
      defmodule VendingMachineWeb.PageLive do
        use VendingMachineWeb, :live_view

        on_mount {VendingMachineWeb.UserAuth, :mount_current_user}
        ...
      end

  Or use the `live_session` of your router to invoke the on_mount callback:

      live_session :authenticated, on_mount: [{VendingMachineWeb.UserAuth, :ensure_authenticated}] do
  ...
  def check_token_validity(token) do
    case Accounts.get_user_by_session_token(token) do
      nil -> {:error, "Invalid/Expired token"}
      user -> {:ok, user}
    end
  end
